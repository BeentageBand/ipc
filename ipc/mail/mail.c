...
static void mail_copy(union Mail * const dest, union Mail * const src);
...
void mail_override(union Mail_Class * const clazz)
{
  ...
  clazz->Class.copy = (Class_Copy_T) mail_copy;
...
void mail_copy(union Mail * const dest, union Mail * const src)
{
  Mail_populate(dest, src->mid, src->sender, src->receiver, src->payload, src->pay_size);
}
